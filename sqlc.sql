...
select title, count(Seasons.season) from Series, Seasons where Seasons.seriesId = Series.seriesId group by title;
-- query 08) return the title of the series that had at leason one season with less than 10 episodes sorted by title
select distinct Series.title from Seasons, Series where Seasons.episodes < 10 AND Seasons.seriesId = Series.seriesId order by 1;
-- query 09) return the title of all series watched by user 'Joe' sorted by title
select Series.title from Watches, Series Where Watches.userID = '1' AND Watches.seriesId = Series.seriesId order by 1;
-- query 10) return the title of all series that had 'Keri Russell' sorted by title
select Series.title from Acts, Series where Acts.actorId = '1' AND Acts.seriesId = Series.seriesId order by 1;
